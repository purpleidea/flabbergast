...
function checkNoMatches() {
  ...
  for (let it = 0; it < searchlist.length; it++) {
    if (
      window.getComputedStyle(searchlist[it]).getPropertyValue("display") !=
      "none"
    ) {
      any_visible = true;
      break;
  ...
  document.getElementById("nomatches").style.display = any_visible
    ? "none"
    : "block";
}
...
export function expandAll(id) {
  for (
    let target = document.getElementById(id);
    target != null && target.nodeName == "DT";
    target = target.parentElement.parentElement.previousElementSibling
  ) {
...
export function setup(terms, libraryNames) {
  function showTerm(term, prefix) {
    const termcss = document.getElementById("termcss");
    const visible = [];
    const hidden = [`#terms a.${prefix}none`];
    for (let known_term of terms) {
      (known_term == term ? visible : hidden).push(
        `#terms a.${prefix}_${known_term}`
      );
    }
    termcss.innerHTML =
      cssForArray(
        visible,
        (prefix == "def" ? "color: #4F94CD; " : "") +
          "font-weight: bold; display: block !important;"
      ) + cssForArray(hidden, "display: none;");
  ...
  return {
    ...
    searchChange: () => {
      ...
      if (searchbox.value.length == 0) {
        ...
        if (
          exact_defs.length > 0 ||
          exact_uses.length > 0 ||
          starts_defs.length > 0 ||
          starts_uses.length > 0
        ) {
          const showPartials =
            getSelection("showPartials") ||
            (exact_defs.length == 0 && exact_uses.length == 0);
          cssParts = [
            {
              items: exact_defs,
              css:
                "font-weight: bold; color: #4F94CD; display: block !important;"
            },
            {
              items: exact_uses,
            ...
            },
            {
              items: starts_defs,
              css: showPartials
                ? "color: #4F94CD; display: block !important;"
                : "display: none;"
            },
            {
              items: starts_uses,
        ...
        termcss.innerHTML =
          cssParts.map(part => cssForArray(part.items, part.css)).join("") +
          (getSelection("hideExternals")
    ...
    init: () => {
      ...
      const showLoadError = (context, message) => {
        ...
        p.appendChild(
          document.createTextNode(`${context}: ${message || "Unknown error"}`)
        );
      ...
      const unref = () => {
        ...
        const sortedLibraries = libraries.sort((a, b) => {
          ...
          for (
            let i = 0;
            result == 0 && i < a.nameParts.length && i < b.nameParts.length;
            i++
          ) {
        ...
        sortedLibraries
          .filter(library => library.links)
          .forEach(library => searchlistdiv.appendChild(library.links));
      ...
      const downloadLibrary = info => {
        ...
        request.addEventListener("load", () => {
          info.links = termsForExternal.transformToFragment(
            request.responseXML,
            document
          );
          const nsResolver = request.responseXML.createNSResolver(
            request.responseXML.documentElement
          );
          const newLibraries = request.responseXML.evaluate(
            "//o_0:ref/text()[not(contains(., 'interop'))]",
            request.responseXML.documentElement,
            nsResolver,
          ...
          for (
            let newNameNode = newLibraries.iterateNext();
            newNameNode;
            newNameNode = newLibraries.iterateNext()
          ) {
...
export function showHide(roller) {
