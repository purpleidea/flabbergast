...
namespace Flabbergast
{
    ...
    {
        private static bool Discover(string directory, int trim, string github, bool verbose, string output_root,
            ErrorCollector collector)
        {
            ...
            foreach (var file in Directory.EnumerateFiles(directory, "*.o_0"))
            {
                ...
                var output_filename = Path.Combine(output_root,
                    "doc-" + file_fragment.Replace(Path.DirectorySeparatorChar, '-') + ".xml");
                if (verbose) Console.WriteLine(file);
                var parser = Parser.Open(file);
                var doc = parser.DocumentFile(collector, uri, github);
                if (doc != null) doc.Save(output_filename);
                else success = false;
            }
            return success;
        }
        ...
        public static int Main(string[] args)
        {
            ...
            foreach (var dir in directories)
                if (Directory.Exists(dir))
                    Discover(dir, dir.Length + (dir[dir.Length - 1] == Path.DirectorySeparatorChar ? 0 : 1), github,
                        verbose, output_root, collector);
            return success ? 0 : 1;
        }
    }
}
