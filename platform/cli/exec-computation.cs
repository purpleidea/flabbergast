...
namespace Flabbergast
{
    ...
    {
        public static readonly Future INSTANCE = new BlackholeFuture();
    ...
    public abstract class InterlockedLookup : Future
    {
        ...
        public static readonly Frame[] Days =
            MakeFrames(new[] { "sunday", "monday", "tuesday", "wednesday", "thrusday", "friday", "saturday" },
                CultureInfo.CurrentCulture.DateTimeFormat.AbbreviatedDayNames,
                CultureInfo.CurrentCulture.DateTimeFormat.DayNames);

        public static readonly Frame[] Months = MakeFrames(
            new[]
            {
        ...
        public static readonly Dictionary<string, Func<DateTime, object>> TIME_ACCESSORS =
            new Dictionary<string, Func<DateTime, object>>
            {
                ...
                {
                    "week",
                    d => (long) new GregorianCalendar().GetWeekOfYear(d, CalendarWeekRule.FirstDay, DayOfWeek.Sunday)
                },
        ...
        public InterlockedLookup(TaskMaster task_master, SourceReference source_reference, Context context) : base(
            task_master)
        {
        ...
        private static Matcher<T> FindMatcher<T>(string custom_error)
        {
            ...
            if (underlying_type == typeof(double))
                return (object x, Action<T> writer, out string error) =>
                {
                    double? d;
                    if (x is double)
            ...
            if (underlying_type != null && (underlying_type == typeof(long) || underlying_type == typeof(bool)))
                return (object x, Action<T> writer, out string error) =>
                {
            ...
            if (typeof(T) == typeof(long) || typeof(T) == typeof(bool) || typeof(T) == typeof(byte[]) ||
                typeof(T) == typeof(Frame) || typeof(T) == typeof(Template) || typeof(T) == typeof(object))
                return (object x, Action<T> writer, out string error) =>
                {
                    if (x is T)
                    {
            ...
            return (object input_result, Action<T> writer, out string error) =>
            {
                ...
                {
                    var backing = ((ReflectedFrame)input_result).Backing;
                    if (backing is T)
                    {
                        writer((T)backing);
                        error = null;
        ...
        public object CorrectOutput<R>(Func<R> compute)
        {
            try
            {
                ...
                if (typeof(R) == typeof(DateTime))
                    return ReflectedFrame.Create(task_master, (DateTime)(object)output, TIME_ACCESSORS);
                return output;
            }
            catch (Exception e)
            {
        ...
        public abstract class BaseSink<T>
        {
            ...
            public void Allow<X>(Func<X, T> converter, string custom_error = null)
            {
                ...
                Add<X>((object input, Action<T> writer, out string error) =>
                    matcher(input, x => writer(converter(x)), out error));
            }
            ...
            public void AllowDefault(string custom_error = null)
            {
                ...
                {
                    var timeObj = AllowObject<Holder<long>>(x =>
                        (T)(object)new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc).AddSeconds(x.Value));
            ...
            public ObjectSink<X> AllowObject<X>(Func<X, T> converter) where X : new()
            {
                ...
                Add<Frame>((object input, Action<T> writer, out string error) =>
                    obj_sink.Matcher(input, x => writer(converter(x)), out error));
                return obj_sink;
            }
            ...
            protected void Dispatch(object input_result, Action<T> writer)
            {
                ...
                owner.task_master.ReportOtherError(owner.source_reference, custom_error ?? string.Format(
                                                                               "“{0}” has type {1} but expected {2}.",
                                                                               names,
                                                                               SupportFunctions.NameForType(
                                                                                   input_result.GetType()),
                                                                               types.Keys.Select(type =>
                                                                                       SupportFunctions.NameForType(
                                                                                           type))
                                                                                   .Aggregate((l, r) =>
                                                                                       l + " or " + r)));
        ...
        public class ListSink<T> : BaseSink<T>
        {
            ...
            protected override void HandleResult(object input_result)
            {
                if (!(input_result is Frame))
                {
                    owner.task_master.ReportOtherError(owner.source_reference,
                        string.Format("“{0}” has type {1} but expected Frame.", names));
                    return;
                }
            ...
            internal bool Matcher(object input, Action<T> writer, out string error)
            {
                ...
                foreach (var entry in writers)
                {
                    ...
                    if (!frame.GetOrSubscribe(entry.Key, arg_result =>
                    {
                        ...
                        owner.task_master.ReportOtherError(frame.SourceReference,
                            string.Format("Attribute “{0}” is not defined in frame.", entry.Key));
                }
                return true;
            }
