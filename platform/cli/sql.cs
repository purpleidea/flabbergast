...
namespace Flabbergast
{
    ...
    {
        private static readonly Dictionary<Type, Unpacker> unpackers = new Dictionary<Type, Unpacker>();
        ...
        static DbQuery()
        {
            ...
            AddUnpacker((rs, position, task_master) => rs.GetInt64(position), typeof(byte), typeof(sbyte),
            ...
            AddUnpacker(
                (rs, position, task_master) =>
                    ReflectedFrame.Create(task_master, rs.GetDateTime(position), TIME_ACCESSORS), typeof(DateTime));
            AddUnpacker((rs, position, task_master) =>
            {
            ...
            AddUnpacker((rs, position, task_master) =>
            {
                ...
                if (result is sbyte || result is short || result is ushort || result is int || result is uint ||
                    result is long || result is ulong)
                    return (long)result;
                if (result is bool)
                    return result;
                ...
                if (result is DateTime)
                    return ReflectedFrame.Create(task_master, rs.GetDateTime(position), TIME_ACCESSORS);
                if (result is byte[])
                    return result;
                return Unit.NULL;
            }, typeof(object));
        ...
        {
            return (rs, frame, task_master) =>
                frame.Set(name, rs.IsDBNull(position) ? Unit.NULL : unpacker(rs, position, task_master));
        }

        private static void AddUnpacker(Unpacker unpacker, params Type[] sql_types)
        {
        ...
        protected override Frame ComputeResult()
        {
            ...
            for (var col = 0; col < reader.FieldCount; col++)
            {
                ...
                if (reader.GetName(col).StartsWith("$"))
                {
                    ...
                    retrievers.Add((rs, frame, _task_master) => frame.Set(attr_name,
                        rs.IsDBNull(column)
                            ? Precomputation.Capture(Unit.NULL)
                            : Lookup.Do(rs.GetString(column).Split('.'))));
                    continue;
                }
                Unpacker unpacker;
            ...
            for (var it = 1; reader.Read(); it++)
            {
                var frame = new MutableFrame(task_master,
                    new JunctionReference(string.Format("SQL template instantiation row {0}", it), "<sql>", 0, 0, 0, 0,
                        source_reference, row_tmpl.SourceReference), Context.Append(list.Context, row_tmpl.Context),
                    list);
    ...
    {
        private static readonly Dictionary<string, Func<DbConnection, object>> connection_hooks =
            new Dictionary<string, Func<DbConnection, object>>
            {
        ...
        public ResourcePathFinder Finder { get; set; }

        public string UriName => "ADO.NET gateway";
        ...
        public Future ResolveUri(TaskMaster task_master, string uri, out LibraryFailure reason)
        {
            ...
            try
            {
                ...
                if (first_colon >= uri.Length)
                    return new FailureFuture(task_master, new ClrSourceReference(),
                        "Bad provider in URI “" + uri + "”.");
                var provider = uri.Substring(4, first_colon - 4);
                var question_mark = first_colon;
                ...
                if (question_mark < uri.Length - 1)
                    foreach (var param_str in uri.Substring(question_mark + 1).Split('&'))
                    {
                ...
                connection_proxy.Set("provider",
                    new SimpleStringish(plus_position == -1 ? provider : provider.Substring(0, plus_position)));
        ...
        internal static bool ParseUri(string uri_fragment, DbConnectionStringBuilder builder, string host_param,
            string port_param, string user_param, string password_param, string database_param, out string err)
        {
        ...
        internal static bool ParseUri(string uri_fragment, DbConnectionStringBuilder builder, string user_param,
            string password_param, HandleParams handle_params, out string err)
        {
            err = null;
            var host_start = 0;
            var user_end = 0;
            while (user_end < uri_fragment.Length &&
                   (char.IsLetterOrDigit(uri_fragment[user_end]) || uri_fragment[user_end] == '_'))
                user_end++;
            ...
            switch (uri_fragment[user_end])
            {
                ...
                case ':':
                    ...
                    var password_end = user_end + 1;
                    while (password_end < uri_fragment.Length && "/@".IndexOf(uri_fragment[password_end]) == -1)
                        password_end++;
            ...
            handle_params(uri_fragment.Substring(host_start, host_end - host_start), port,
                uri_fragment.Substring(db_start));
            return true;
        }
