...
namespace Flabbergast
{
    public class Completables
    {
        ...
        {
            "args", "value", "Append", "Bool", "By", "Container", "Drop", "Each", "Else", "Enforce", "Error", "False",
            "Finite", "Float", "FloatMax", "FloatMin", "For", "Frame", "From", "GenerateId", "Id", "If", "In",
            "Infinity", "Int", "IntMax", "IntMin", "Is", "Length", "Let", "Lookup", "NaN", "Name", "Now", "Null",
            "Order", "Ordinal", "Reduce", "Required", "Reverse", "Select", "Str", "Template", "Then", "This", "Through",
    ...
    public class REPL
    {
        ...
        public static int Main(string[] args)
        {
            ...
            var assembly_builder =
                AppDomain.CurrentDomain.DefineDynamicAssembly(new AssemblyName("Repl"), AssemblyBuilderAccess.Run);
            ...
            resource_finder.PrependPath(Path.Combine(
                files.Count == 1 ? Path.GetDirectoryName(Path.GetFullPath(files[0])) : Environment.CurrentDirectory,
                "lib"));
            resource_finder.AddDefault();
            task_master.AddAllUriHandlers(resource_finder,
                LoadRule.Interactive | (use_precompiled ? LoadRule.Precompiled : 0));
            ...
            while (run && (s = line_editor.Edit(id + "‽ ", "")) != null)
            {
                ...
                {
                    object result = null;
                    var computation = (Future)Activator.CreateInstance(run_type,
                        new object[]
                        {
                            task_master, original, current, update_current, (ConsumeResult) (output => result = output),
                            (ConsumeResult) Console.WriteLine
                        });
        ...
        private static void Print(object result, string prefix, Dictionary<Frame, int> seen,
            Dictionary<SourceReference, bool> seen_srcref, ref int counter)
        {
            if (result == null)
            {
                Console.WriteLine("∅");
