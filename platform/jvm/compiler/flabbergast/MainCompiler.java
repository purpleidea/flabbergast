...
public class MainCompiler {
  ...
  public static void main(String[] args) {
    ...
    if (result.hasOption('h')) {
      ...
      System.err.println("Compile a Flabbergast file for use as a library.");
    ...
    if (files.length == 0) {
      System.err.println("Perhaps you wish to compile some source files?");
      System.exit(1);
      return;
    }
    ErrorCollector collector = new ConsoleCollector();
    CompilationUnit<Boolean> unit = new WriterCompilationUnit(result.hasOption('F'));
    for (String filename : files) {
      try {
        Parser parser = Parser.open(filename);
        parser.setTrace(result.hasOption('t'));
        String file_root =
            ("flabbergast/library/" + removeExtensions(filename, ".o_0", ".jo_0"))
  ...
  private static String removeExtensions(String filename, String... extensions) {
