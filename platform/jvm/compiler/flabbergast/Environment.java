...
class Environment implements CodeRegion {
  ...
  public interface SpecialGenerate<T> {
    ...
    void invoke(T item, Generator.ParameterisedBlock<LoadableValue> result_handler)
        throws Exception;
  }
  ...
  boolean force_back;
  Map<AstNode, Entry<TypeSet, Boolean>> intrinsics =
      new HashMap<AstNode, Entry<TypeSet, Boolean>>();
  ...
  public Environment(
    ...
    this.parent = parent;
    priority = (parent == null ? 0 : parent.getPriority()) + (force_back ? 1 : 2);
  ...
  BoundNameInfo addMask(String name, TypeableElement expression) {
    if (children.containsKey(name)) {
      throw new UnsupportedOperationException(
          "The name " + name + " already exists in the environment.");
    }
  ...
  TypeSet ensureIntrinsic(
    ...
    if (intrinsics.containsKey(node)) {
      ...
      intrinsics.put(node, new AbstractMap.SimpleEntry<TypeSet, Boolean>(type, must_unbox));
      return type;
    }
  }

  <T> void generateLookupCache(
      final Generator generator,
      List<T> specials,
      SpecialGenerate<T> special_transform,
      LookupCache current,
    ...
    if (specials != null) {
      FieldValue child_context = generator.makeField("anon_ctxt", Context.class);
      final FieldValue child_frame = generator.makeField("anon_frame", MutableFrame.class);
      generator.getBuilder().visitVarInsn(Opcodes.ALOAD, 0);
      generator.getBuilder().visitTypeInsn(Opcodes.NEW, getInternalName(MutableFrame.class));
      ...
      generator
          ...
          .visitMethodInsn(
              ...
              org.objectweb.asm.Type.getConstructorDescriptor(
                  MutableFrame.class.getConstructors()[0]));
      ...
      generator
          ...
          .visitMethodInsn(
              ...
              "prepend",
              Generator.makeSignature(Context.class, Frame.class, Context.class));
      ...
      for (T entry : specials) {
        ...
        final RestrictableType restrictable = special_transform.getRestrictableType(entry);
        final FieldValue field =
            generator.makeField("special$" + restrictable.getName(), Object.class);
        ...
        special_transform.invoke(
            ...
            new Generator.ParameterisedBlock<LoadableValue>() {
              ...
              public void invoke(LoadableValue result) throws Exception {
                child_frame.load(generator);
                generator.getBuilder().visitLdcInsn(restrictable.getName());
                generator.loadReboxed(result, Object.class);
                generator.visitMethod(
                    MutableFrame.class.getMethod("set", String.class, Object.class));
                generator.copyField(result, field);
                generator.jumpToState(next);
                known_types.add(TypeSet.fromNative(result.getBackingType()));
              }
            });
        generator.markState(next);
        lookup_results.add(
            new LoadableCache(
                field, restrictable.getRestrictedType().intersect(known_types),
    ...
    if (current != null) {
      ...
      for (NameInfo info : all_children) {
        String current_narrow_error = info.checkValidNarrowing(base_lookup_cache, current);
        if (narrow_error != null && current_narrow_error != null) {
          narrow_error = String.format("%s\n%s", narrow_error, current_narrow_error);
      ...
      if (narrow_error != null) {
        ...
        generator
            ...
            .visitMethodInsn(
                ...
                "reportOtherError",
                Generator.makeSignature(null, SourceReference.class, String.class));
    ...
    for (LoadableCache lookup_result : lookup_results) {
      ...
      base_lookup_cache.set(lookup_result.getNameInfo(), lookup_result.getValue());
    }
    for (LoadableCache lookup_result : lookup_results) {
      if (!(lookup_result.getSinglyTyped() && !lookup_result.getDirectCopy())) {
        continue;
      }
      base_lookup_cache.set(
          lookup_result.getNameInfo(),
          new AutoUnboxValue(lookup_result.getValue(), lookup_result.getTypes().get(0)));
      Label label = new Label();
      lookup_result.getValue().load(generator);
      Class<?> required_type = Generator.getBoxedType(lookup_result.getTypes().get(0));
      generator.getBuilder().visitTypeInsn(Opcodes.INSTANCEOF, getInternalName(required_type));
      generator.getBuilder().visitJumpInsn(Opcodes.IFNE, label);
      generator.emitTypeError(
          source_reference,
          String.format(
              "Expected type %s for “%s”, but got %s.",
              SupportFunctions.nameForClass(required_type),
              lookup_result.getNameInfo().getName(),
              "%s"),
    ...
    if (generator.getPaths() > 200 && !combinatorial_explosion) {
      System.err.printf(
          "%s:%d:%d-%d:%d: There are %d type-derived flows in the generated code. This will be slow to compile.\n",
          file_name, start_row, start_column, end_row, end_column, generator.getPaths());
      combinatorial_explosion = true;
    }
    generateLookupPermutation(
        generator, context, base_lookup_cache, 0, permutable_caches, source_reference, block);
    generator.setPaths(old_paths);
  }

  private void generateLookupPermutation(
      Generator generator,
      LoadableValue context,
      LookupCache cache,
      int index,
      List<LoadableCache> values,
      LoadableValue source_reference,
      Block block)
      throws Exception {
    ...
    for (int it = 0; it < labels.length; it++) {
      ...
      generator
          ...
          .visitTypeInsn(
              Opcodes.INSTANCEOF,
              getInternalName(Generator.getBoxedType(values.get(index).getTypes().get(it))));
    ...
    for (int type_it = 0; type_it < values.get(index).getTypes().size(); type_it++) {
      ...
      error_message.append(
          SupportFunctions.nameForClass(values.get(index).getTypes().get(type_it)));
    }
    error_message.append(" for “");
    error_message.append(values.get(index).getNameInfo().getName());
    error_message.append("”, but got %s.");
    generator.emitTypeError(
        source_reference, error_message.toString(), values.get(index).getValue());
    for (int it = 0; it < labels.length; it++) {
      generator.getBuilder().visitLabel(labels[it]);
      generator.debugPosition(this);
      LookupCache sub_cache = new LookupCache(cache);
      sub_cache.set(
          values.get(index).getNameInfo(),
          new AutoUnboxValue(values.get(index).getValue(), values.get(index).getTypes().get(it)));
      MethodVisitor builder = generator.getBuilder();
      generateLookupPermutation(
          generator, context, sub_cache, index + 1, values, source_reference, block);
  ...
  void intrinsicDispatch(
    ...
    for (Class<Object> type : types) {
      ...
      MethodVisitor builder = generator.getBuilder();
      builder.visitTypeInsn(Opcodes.INSTANCEOF, getInternalName(Generator.getBoxedType(type)));
    ...
    error_message.append(", but got %s.");
    generator.emitTypeError(source_reference, error_message.toString(), original);
  }
  ...
  private NameInfo lookback(String name) {
    ...
    NameInfo copy_info = new CopyFromParentInfo(this, name, original, force_back);
    children.put(name, copy_info);
    return copy_info;
  }

  public NameInfo lookup(
      ErrorCollector collector, Iterable<? extends CharSequence> names, Ptr<Boolean> success) {
