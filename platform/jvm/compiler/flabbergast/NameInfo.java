...
abstract class NameInfo {
  ...
  protected LoadableValue generateLookupField(
    ...
    builder.visitMethodInsn(
        ...
        getInternalName(Lookup.class),
        "<init>",
        org.objectweb.asm.Type.getConstructorDescriptor(Lookup.class.getConstructors()[0]));
  ...
  public abstract LoadableCache load(
      Generator generator, LoadableValue source_reference, LoadableValue context) throws Exception;

  NameInfo lookup(
      ErrorCollector collector, Iterator<? extends CharSequence> names, Ptr<Boolean> success) {
    NameInfo info = this;
    while (names.hasNext()) {
