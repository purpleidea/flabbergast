...
class CopyFromParentInfo extends NameInfo {
  ...
  public CopyFromParentInfo(
      Environment environment, String name, NameInfo source, boolean force_back) {
  ...
  public String checkValidNarrowing(LookupCache next, LookupCache current) {
    if (current.has(source)) {
      ...
      TypeSet union_type = TypeSet.fromNative(parent_value.getBackingType());
      if (mask.intersect(union_type).isEmpty()) {
        return String.format("Value for “%s” must be to %s, but it is %s.", name, mask, union_type);
      } else {
  ...
  public void createChild(
    ...
    if (source.hasName(name)) {
      children.put(
          name,
          new CopyFromParentInfo(
              environment, root + "." + name, source.lookup(collector, name, success), force_back));
    } else {
  ...
  public LoadableCache load(
    ...
    LoadableCache source_cache = source.load(generator, source_reference, context);
    return new LoadableCache(
        source_cache.getValue(), source_cache.getPossibleTypes().intersect(mask), this, must_unbox);
  }
  ...
  public boolean needsLoad(LookupCache current) {
    return current == null
        || !current.has(source)
        || must_unbox && current.get(source).getBackingType() == Object.class;
  }
}
