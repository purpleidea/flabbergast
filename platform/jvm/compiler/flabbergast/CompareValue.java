  ...
  public void load(MethodVisitor generator) throws Exception {
    if (left.getBackingType() == boolean.class) {
      ...
      generator.visitMethodInsn(
          Generator.isNumeric(left.getBackingType()) ? Opcodes.INVOKESTATIC : Opcodes.INVOKEVIRTUAL,
          getInternalName(Generator.getBoxedType(left.getBackingType())),
          Generator.isNumeric(left.getBackingType()) ? "compare" : "compareTo",
          Generator.makeSignature(
              int.class,
              Generator.isNumeric(left.getBackingType()) ? left.getBackingType() : null,
              right.getBackingType()));
      generator.visitInsn(Opcodes.ICONST_1);
      generator.visitMethodInsn(
          Opcodes.INVOKESTATIC,
          getInternalName(Math.class),
          "min",
