package flabbergast;

import org.objectweb.asm.MethodVisitor;

class FloatConstant extends LoadableValue {
  public static final FloatConstant INFINITY = new FloatConstant(Double.POSITIVE_INFINITY);
  public static final FloatConstant NAN = new FloatConstant(Double.NaN);
  private final double number;

  public FloatConstant(double number) {
    this.number = number;
  }

  @Override
  public Class<?> getBackingType() {
    return double.class;
  }

  @Override
  public void load(MethodVisitor generator) {
    generator.visitLdcInsn(number);
  }
}
