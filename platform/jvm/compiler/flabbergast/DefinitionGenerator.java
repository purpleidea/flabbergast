...
class DefinitionGenerator extends Generator {
  ...
  DefinitionGenerator(
    ...
    super(node, owner, type_builder, class_name, root_prefix, owner_externals);

    initial_source_reference = makeField("source_reference", SourceReference.class);
    ...
    FieldValue original = has_original ? makeField("original", Future.class) : null;

    Class<?>[] construct_params =
        new Class<?>[] {
          TaskMaster.class,
    ...
    FieldValue[] initial_information =
        new FieldValue[] {
          initial_source_reference, initial_context, initial_self, initial_container, original
        };
    ...
    MethodVisitor ctor_builder =
        type_builder.visitMethod(
            Opcodes.ACC_PUBLIC, "<init>", makeSignature(null, construct_params), null, null);
    ...
    ctor_builder.visitMethodInsn(
        ...
        getInternalName(Future.class),
        "<init>",
        Type.getConstructorDescriptor(Future.class.getConstructors()[0]));
    ...
    ClassVisitor init_class =
        owner.defineClass(
            Opcodes.ACC_PUBLIC,
            init_name,
            Object.class,
            has_original ? ComputeOverride.class : ComputeValue.class);
    MethodVisitor init_ctor =
        init_class.visitMethod(Opcodes.ACC_PUBLIC, "<init>", "()V", null, null);
    init_ctor.visitCode();
    init_ctor.visitVarInsn(Opcodes.ALOAD, 0);
    init_ctor.visitMethodInsn(
        Opcodes.INVOKESPECIAL, getInternalName(Object.class), "<init>", "()V");
    ...
    // create a delegate.
    MethodVisitor init_builder =
        init_class.visitMethod(
            Opcodes.ACC_PUBLIC,
            "invoke",
            makeSignature(Future.class, construct_params),
            null,
            null);
    ...
    if (has_original) {
      ...
      init_builder.visitLdcInsn("Cannot perform override. No value in source frame to override!");
      init_builder.visitMethodInsn(
    ...
    init_builder.visitMethodInsn(
        Opcodes.INVOKESPECIAL, class_name, "<init>", makeSignature(null, construct_params));
    ...
    if (has_original) {
      ...
      visitMethod(Future.class.getMethod("listen", ConsumeResult.class));
      stopInterlock();
    } else {
  ...
  public DelegateValue getInitialiser() {
    return new DelegateValue(
        class_name + "$Initialiser",
        initial_original == null ? ComputeValue.class : ComputeOverride.class);
  }
