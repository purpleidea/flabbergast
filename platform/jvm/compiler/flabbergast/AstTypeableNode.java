...
abstract class AstTypeableNode extends AstNode {
  static TypeSet checkReflectedMethod(
      ErrorCollector collector,
      AstNode where,
      List<Method> methods,
      List<? extends TypeableElement> arguments,
      TypeSet return_type,
      Ptr<Boolean> success) {
    ...
    if (candidate_methods.size() == 0) {
      collector.reportExpressionTypeError(where, return_type, total_union);
      return TypeSet.EMPTY;
    }
    /*
      ...
      for (Method method : methods) {
        Class<?> param_type =
            Modifier.isStatic(method.getModifiers())
                ...
                : (it == 0 ? method.getDeclaringClass() : method.getParameterTypes()[it - 1]);
        candidate_parameter_type = candidate_parameter_type.union(Type.fromNative(param_type));
      }
      arguments.get(it).ensureType(collector, candidate_parameter_type, success, true);
    }
    return candiate_return;
  }

  static Class<?> getReflectedType(
      ErrorCollector collector, AstNode where, String type_name, Ptr<Boolean> success) {
    try {
      return Class.forName(type_name);
    } catch (ClassNotFoundException e) {
      success.set(false);
      collector.reportRawError(where, "No such type " + type_name + " found.");
      return null;
    }
  }

  static Field reflectField(
      ErrorCollector collector,
      AstNode where,
      String type_name,
      String field_name,
      Ptr<Boolean> success) {
    Class<?> reflected_type = getReflectedType(collector, where, type_name, success);
    ...
    try {
      ...
      if (!Modifier.isStatic(field.getModifiers())) {
        success.set(false);
        collector.reportRawError(
            where, "The field " + field_name + " in  type " + type_name + " has is not static.");
        return null;
      }
      return field;
      ...
      success.set(false);
      collector.reportRawError(
          where, "The type " + type_name + " has no field named " + field_name);
      return null;
    } catch (SecurityException e) {
      success.set(false);
      collector.reportRawError(
          where, "The field " + field_name + " in  type " + type_name + " is not public.");
      return null;
    }
  }
  ...
  static void reflectMethod(
    ...
    for (Method method : reflected_type.getMethods()) {
      int adjusted_arity =
          method.getParameterTypes().length + (Modifier.isStatic(method.getModifiers()) ? 0 : 1);
  ...
  public boolean analyse(ErrorCollector collector) {
    ...
    SortedMap<Integer, Set<AstTypeableNode>> sorted_nodes =
        new TreeMap<Integer, Set<AstTypeableNode>>();

    for (AstTypeableNode element : queue) {
      if (!sorted_nodes.containsKey(element.getEnvironmentPriority())) {
        sorted_nodes.put(element.getEnvironmentPriority(), new HashSet<AstTypeableNode>());
      }
  ...
  abstract Environment propagateEnvironment(
      ErrorCollector collector,
      List<AstTypeableNode> queue,
      Environment environment,
      Ptr<Boolean> success);
}
