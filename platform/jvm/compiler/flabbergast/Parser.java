...
public class Parser {
  ...
  class Position {
     ...
     *
     * @param result The result to cache, or null if parsing failed.
     */
    <T extends AstNode> void cache(int start_index, T result, Class<T> clazz) {
      if (!cache.containsKey(start_index)) {
        cache.put(start_index, new HashMap<Class<? extends AstNode>, Parser.Memory>());
      }
     ...
     * @param result The result to cache, or null if parsing failed.
     */
    public <T extends AstNode> void cache(String name, int start_index, T result) {
      if (!alternate_cache.containsKey(start_index)) {
        alternate_cache.put(start_index, new HashMap<String, Parser.Memory>());
      }
      Memory memory = new Memory();
      memory.result = result;
      memory.index = index;
    ...
    <U extends AstNode, T extends U> boolean checkCache(Ptr<U> result, Class<T> clazz) {
      if (cache.containsKey(index) && cache.get(index).containsKey(clazz)) {
        ...
        if (trace) {
          ...
          System.out.printf(
              "%d:%d %s %s\n",
              row, column, (result.get() == null ? " M " : " H "), clazz.getName());
          trace_depth++;
        }
        return true;
      }
    ...
    <T extends AstNode> boolean checkCache(String name, Ptr<T> result) {
      if (alternate_cache.containsKey(index) && alternate_cache.get(index).containsKey(name)) {
        ...
        if (trace) {
          ...
          System.out.printf(
              "%d:%d %s %s\n", row, column, (result.get() == null ? " M " : " H "), name);
          trace_depth++;
        }
        return true;
      }
    ...
    void nameConstraint(String name) {
      error_collector.reportParseError(
          file_name,
          index,
          row,
          column,
  ...
  public static Parser open(String filename) throws IOException {
    return new Parser(
        filename, new Scanner(new File(filename), "UTF-8").useDelimiter("\\Z").next());
  }
  ...
  Map<Integer, Map<Class<? extends AstNode>, Memory>> cache =
      new HashMap<Integer, Map<Class<? extends AstNode>, Memory>>();
  int column;
   ...
   *
   * <p>This may have a value even if the parse was successful.
   */
  String message;
  ...
  public Document documentFile(ErrorCollector collector, String lib_name, String github)
      throws ParserConfigurationException {
    Ptr<file> result = new Ptr<file>();
    Ptr<Position> position = new Ptr<Position>(new Position(collector));
    if (file.parseRule_Base(position, result) && position.get().isFinished()) {
  ...
  public <T> T parseFile(ErrorCollector collector, CompilationUnit<T> unit, String type_name)
      throws Exception {
    final Ptr<file> result = new Ptr<file>();
    Ptr<Position> position = new Ptr<Position>(new Position(collector));
    if (file.parseRule_Base(position, result) && position.get().isFinished()) {
      if (result.get().analyse(collector)) {
        return unit.createRootGenerator(
            result.get(),
            type_name,
            new Generator.Block() {

              @Override
              public void invoke(final Generator generator) throws Exception {
                result
                    .get()
                    .generate(
                        generator,
                        new ParameterisedBlock<LoadableValue>() {
                          ...
                          @Override
                          public void invoke(LoadableValue value) throws Exception {
                            generator.doReturn(value);
                          }
                        });
              }
            });
  ...
  <T> boolean parseIntoList(Ptr<Position> position, List<T> result, ParseRule<T> rule) {
    Ptr<T> obj = new Ptr<T>();
  ...
  public <T> T parseRepl(ErrorCollector collector, CompilationUnit<T> unit, String type_name)
      throws Exception {
    final Ptr<repl> result = new Ptr<repl>();
    Ptr<Position> position = new Ptr<Position>(new Position(collector));
    if (repl.parseRule_Base(position, result) && position.get().isFinished()) {
      if (result.get().analyse(collector)) {
        return unit.createReplGenerator(
            result.get(),
            type_name,
