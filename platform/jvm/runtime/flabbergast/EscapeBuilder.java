...
public class EscapeBuilder extends InterlockedLookup {
    ...
    public static int stringToCodepoint(String str) {
        ...
        if (str.offsetByCodePoints(0, 1) < str.length()) {
            throw new IllegalArgumentException(String.format(
                                                   "String “%s” must be a single character.",
                                                   str));
        }
    ...
    protected void setup() {
        ListSink<Transformation> transformations_lookup = findAll(Transformation.class, input -> input.values().stream().forEach(x->x.accept(this)));
    ...
    protected void resolve() {
        ranges.sort(null);
        Template tmpl = new Template(new BasicSourceReference("Make escape template", "<escape>", 0, 0, 0, 0, source_reference), context, self);
