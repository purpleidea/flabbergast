...
public abstract class InterlockedLookup extends Future {
    ...
    public static final Map<String, Transform<ZonedDateTime>> TIME_ACCESSORS = new HashMap<String, Transform<ZonedDateTime>>();
    static {
    ...
    private static <T> Matcher<T> findMatcher(Class<? extends T> clazz, boolean allow_null, String custom_error) {
        ...
        } else if (clazz == Long.class || clazz == Boolean.class || clazz == byte[].class || clazz == Frame.class || clazz == Template.class || clazz == Object.class) {
            return (x, writer, error) -> {
                if (allow_null && x == Unit.NULL) {
                    writer.accept(null);
                    error.set(null);
    ...
    public abstract class BaseSink<T> implements ConsumeResult {
        ...
        public <X>void allow(Class<? extends X> clazz, Function<X, T> converter, boolean allow_null, String custom_error) {
            Matcher<X> matcher =  findMatcher(clazz,  allow_null, custom_error);
            add(clazz, (input, writer, error) -> matcher.invoke(input, x -> writer.accept(converter.apply(x)), error));
        }
        ...
        public void allowDefault(boolean allow_null, String custom_error) {
            ...
            if (clazz == ZonedDateTime.class) {
                ObjectSink<Ptr<Long>> timeObj = allowObject(x -> (T)ZonedDateTime.ofInstant(Instant.ofEpochSecond(x.get()), ZoneId.of("Z")), Ptr::new);
                timeObj.add(Long.class, "epoch", false, Ptr::set, "Time-based tuple must have “epoch” attribute.");
            }
        }

        public <X>ObjectSink<X> allowObject(Function<X, T> converter, Supplier<X> ctor) {
            ObjectSink<X> obj_sink = new ObjectSink<>(ctor);
            add(Frame.class, (input,  writer, error) -> obj_sink.invoke(input, x -> writer.accept(converter.apply(x)), error));
            return obj_sink;
        }
        ...
        protected void dispatch(Object input_result, Consumer<T> writer) {
            ...
            task_master.reportOtherError(source_reference, custom_error != null ? custom_error : String.format("“%s” has type %s but expected %s.", names, SupportFunctions.nameForClass(input_result.getClass()),
                                         types.stream().map(SupportFunctions::nameForClass).collect(Collectors.joining(" or "))));
        }
    ...
    public class ListSink<T> extends BaseSink<T> {
        ...
        public void consume(Object input_result) {
            if (!(input_result instanceof Frame)) {
                task_master.reportOtherError(source_reference, String.format("“%s” has type %s but expected Frame.", names));
                return;
            }
    ...
    public class ObjectSink<T> implements Matcher<T> {
        ...
        public boolean invoke(Object input, Consumer<T> writer, Ptr<String> error) {
            ...
            writers.entrySet().stream().forEach(entry -> {
                if (!frame.getOrSubscribe(entry.getKey(), arg_result -> {
                Ptr<String> writer_error = new Ptr<>();
                    if (entry.getValue().invoke(result, arg_result, writer_error)) {
                        ...
                        task_master.reportOtherError(frame.getSourceReference(), writer_error.get());

                    }
                })) {
                    task_master.reportOtherError(frame.getSourceReference(), String.format("Attribute “%s” is not defined in frame.", entry.getKey()));
                }
            });
            return true;
        }

        public <X>void add(Class<? extends X> clazz, String name, boolean allow_null, BiConsumer<T, X> writer, String custom_error) {
            Matcher<X> matcher = findMatcher(clazz, allow_null, custom_error);
            writers.put(name, (obj, val, error) -> matcher.invoke(val, x -> writer.accept(obj, x), error));
        }
    }
    ...
    public InterlockedLookup(TaskMaster task_master, SourceReference source_reference, Context context) {
        super(task_master);
