...
public class JdbcUriHandler implements UriHandler {
  ...
  private static Map<String, Transform<Connection>> connection_hooks =
      new HashMap<String, Transform<Connection>>();

  static {
    connection_hooks.put(
        "database",
        c -> {
          try {
            return c.getCatalog();
  ...
  public static String parseUri(
    ...
    while (user_end < uri_fragment.length()
        && (Character.isLetterOrDigit(uri_fragment.charAt(user_end))
            || uri_fragment.charAt(user_end) == '_')) {
      user_end++;
    }
    if (user_end == uri_fragment.length()) {
      ...
      switch (uri_fragment.charAt(user_end)) {
        case '@':
          // End of user string.
          properties.setProperty(user_param, uri_fragment.substring(0, user_end));
        ...
        case ':':
          ...
          if (password_end == uri_fragment.length()) {
            ...
            properties.setProperty(
                password_param, uri_fragment.substring(user_end + 1, password_end));
          }
    ...
    while (host_end < uri_fragment.length() && "/:".indexOf(uri_fragment.charAt(host_end)) == -1) {
      // IPv6 address?
      if (uri_fragment.charAt(host_end) == '[') {
        while (host_end < uri_fragment.length() && uri_fragment.charAt(host_end) != ']') {
          host_end++;
        }
      }
      host_end++;
    ...
    if (uri_fragment.charAt(host_end) == ':') {
      host_end++;
      while (host_end < uri_fragment.length() && Character.isDigit(uri_fragment.charAt(host_end))) {
        host_end++;
      }
      if (host_end == uri_fragment.length()) {
        err.set("Missing “/” followed by database in SQL URI.");
    ...
    while (db_end < uri_fragment.length() && uri_fragment.charAt(db_end) != '/') {
      db_end++;
    }
    ...
    if (db_param == null) {
      ...
      properties.setProperty(db_param, uri_fragment.substring(host_end + 1));
    ...
    try {
      ...
      if (first_colon == uri.length()) {
        return new FailureFuture(
            task_master, new JavaSourceReference(), "Bad provider in URI “" + uri + "”.");
      }
      String provider = uri.substring(4, first_colon);
      int question_mark = first_colon;
      while (question_mark < uri.length() && uri.charAt(question_mark) != '?') {
        question_mark++;
      }
      ...
      if (question_mark < uri.length() - 1) {
        for (String param_str : uri.substring(question_mark + 1).split("&")) {
          ...
          if (parts.length != 2) {
            return new FailureFuture(
                task_master, new JavaSourceReference(), "Bad parameter “" + param_str + "”.");
          }
      ...
      String jdbc_uri = JdbcParser.parse(provider, uri_fragment, params, properties, finder, err);
      if (jdbc_uri == null) {
        return new FailureFuture(task_master, new JavaSourceReference(), err.get());
      }
      Connection connection = DriverManager.getConnection(jdbc_uri, properties);
      ...
      ReflectedFrame connection_proxy =
          ReflectedFrame.create(task_master, connection, connection_hooks);
      int plus_position = provider.indexOf('+');
      connection_proxy.set(
          "provider",
          new SimpleStringish(
              plus_position == -1 ? provider : provider.substring(0, plus_position)));
